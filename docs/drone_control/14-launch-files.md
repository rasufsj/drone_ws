...
def generate_launch_description():      # (3)
    return LaunchDescription([
        Node(                            # (4)
            package="tf2_ros",           # (5)
            executable="static_transform_publisher",  # (6)
            name="tf_base_link_to_fcu",  # (7)
            arguments=[                  # (8)
                "0", "0", "0",           # (9)  x y z
                "0", "0", "0",           # (10) roll pitch yaw
                "uav1/base_link",        # (11) frame pai
                "uav1/fcu",              # (12) frame filho
            ],
            output="screen",             # (13)
        ),
    ])
...
|---|-------|-----------|
| 1 | `from launch import LaunchDescription` | Container do launch |
| 2 | `from launch_ros.actions import Node` | Ação de nó |
| 3 | `def generate_launch_description():` | Ponto de entrada |
| 4 | `Node(` | Declara o nó de TF estática |
| 5 | `package="tf2_ros",` | Pacote padrão do ROS 2 para transformações |
| 6 | `executable="static_transform_publisher",` | Nó que publica TF estática com QoS `transient_local` (latched) |
| 7 | `name="tf_base_link_to_fcu",` | Nome único do nó no grafo TF |
| 8 | `arguments=[` | Interface legacy do `static_transform_publisher`: `x y z roll pitch yaw parent child` |
| 9 | `"0", "0", "0",` | Translação x=0, y=0, z=0: frames coincidentes (sem deslocamento) |
| 10 | `"0", "0", "0",` | Rotação roll=0, pitch=0, yaw=0: frames alinhados (sem rotação) |
| 11 | `"uav1/base_link",` | Frame pai na árvore TF |
| 12 | `"uav1/fcu",` | Frame filho — FCU é coincidente com base_link nesta configuração |
| 13 | `output="screen",` | Loga confirmação de publicação da TF no terminal |

### Quando e por que usar

- Usado como **fallback** quando o MAVROS ou o simulador não publica automaticamente
  a TF `uav1/base_link → uav1/fcu`.
- Algoritmos de visão e de localização que consultam `tf2` esperam encontrar
  `uav1/fcu` como filho de `uav1/base_link` na árvore.
- Verificar se já existe antes de usar:
  ```bash
...
def generate_launch_description():
    return LaunchDescription([
        ...
        Node(                                          # (1)
            ...
            name="tf_rgbd_down",                       # (2)
            arguments=[
                "0.153", "0.0", "-0.129",              # (3) x y z
                "0.0", "0.0", "0.0",                   # (4) roll pitch yaw
                "uav1/fcu", "uav1/rgbd_down",          # (5) parent child
            ],
            output="screen",
        ),
        ...
        Node(                                          # (6)
            package="tf2_ros",
            executable="static_transform_publisher",
            name="tf_rgbd_front",                      # (7)
            arguments=[
                "0.181", "0.0", "-0.089",              # (8) x y z
                "0.0", "0.0", "0.0",                   # (9) roll pitch yaw
                "uav1/fcu", "uav1/rgbd_front",         # (10) parent child
            ],
            output="screen",
        ),
    ])
